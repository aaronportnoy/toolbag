...
class Driver(base.Driver):
    ...
    def con_edge(cls, session, (ctx_source,con_source),(ctx_target,con_target)):
        ...
        if ctx_source == 0xffffffffffffffff:
            return
        #print "con_edge: replace into edge (session,source,target,start,end) values(%x, %d, %d, %d, %d) " % (session.id, con_source, con_target, ctx_source, ctx_target)
    ...
    def con_select(cls, session, query):
        ...
        a = query.sqlaq()
        if a:
            q = 'select dataset.address,tag.name,dataset.value from content dataset inner join tag on tag.id=dataset.tag where dataset.id in (select dataset.id from content dataset inner join tag on tag.id=dataset.tag inner join context on context.address=dataset.context where %s) and (%s)'%(query.sqlq(), a)
        else:
            q = 'select dataset.address,tag.name,dataset.value from content dataset inner join tag on tag.id=dataset.tag where dataset.id in (select dataset.id from content dataset inner join tag on tag.id=dataset.tag inner join context on context.address=dataset.context where %s)'%(query.sqlq())
